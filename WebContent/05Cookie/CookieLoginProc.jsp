...
/*
	...
	통해 배열로 받아와야겠지만, 항목이 하나만 있는 경우에는 아래와 같이
	처리가능하다.
*/
...
if("kosmo".equals(userid) && "1234".equals(userpw)){
	...
	//로그인과 쿠키생성이 완료되면 기존 로그인페이지로 이동하게 된다.
	response.sendRedirect("CookieLoginMain.jsp");
}else{
	//로그인에 실패한 경우 리퀘스트영역에 속성을 저장한 후..
	request.setAttribute("ERROR_MSG", "회원이 아닙니다");
	//로그인 페이지로 포워드(전달) 한다.
	request.getRequestDispatcher("CookieLoginMain.jsp").forward(request
			,response);
	//포워드한 페이지는 리퀘스트영역이 공유되므로 위에 저장한 속성을 읽어올수 있다.
}
%>
