   ...
   \${result } : ${result }<br/>
	...
	<c:if test="${param.user ne '' and param.pass ne ''}">	
		<c:if test="${param.user eq 'kosmo' and param.pass eq '1234'}">		
			${param.user }님 로그인하셨습니다.
		</c:if>	
	</c:if> 
	--%>
	<!-- 로그인을 위한 파라미터가 있는 경우에만 검증을 위한 아래문장을 확인함 -->
	<c:if test="${not empty param.user }">
		...
		<br />
		<!--  
		첫번째 if문에서의 결과에 따라 두번째 if문은 반대와 결과를
		가제기 되는 구조로 작성되었다. 즉, loginResult가 true값으로
		설정된다면 두번째 if문에서는 false의 값으로 바뀌게 된다.
		-->
		<c:if test="${param.user eq 'kosmo' and param.pass eq '1234' }" var="LoginCheck">
			${param.user }님 하이룽~~ <br />
		</c:if>
