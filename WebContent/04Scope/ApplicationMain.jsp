...
//리스트 계열의 컬랙션에 DTO객체 저장(ArrayList 와 사용법은 동일함)
...
//맵계열의 컬랙션에 DTO객체 저장(key,value를 통해 객체저장)
...
<body>
	<h2>어플리케이션 영역의 공유</h2>
	<!-- 
		새로운 페이지에 대한 요청이므로 page영역 , request영역 둘다 공유되지않는다
	 -->
	<h3>사용자 클릭에 의한 이동</h3>
	...
	<!-- 
		페이지 던달이 되었으므로 page영역은 공유되지 않으나 request영역은 공유된다.
	 -->
	<h3>포워드로 페이지 전달하기</h3>
	<%
 ...
 외부파일 인클루드 : include 지시어를 통해 페이지를 삽입하면
 			...
 			간주하면된다. 그러므로 page영역, request영역이 공유된다.
  -->
