...
<body>
	...
	폼값을 받을수 있다. 만약 값이 입력되지 않았다면 길이가 0인 String 객체를 
	...
	*/
	...
	if(!(name==null || name.equals(""))){
		...
		/*
		스크리브랫 안에ㅓ JS를 선언할때는 out.println()을 통해서
		아래와같이 사용할수있다 또는 문자열을 연결하여 사용할수도 있따.
		*/
		out.println("<script>");
	...
	/*
	redio는 폼값을 아래 2가지 방법을 모두 사용할수있다
	항목은 여러개이나 실제 선택가능한 항목은 하나이기 떄문이다.
	*/
	//방법1 : text타입과 동일한 형태로 폼값을 받을떄...
	...
	여러개의 항목중 테크가 된 항목만 전송된다 파라미터명이 틀리거나
	...
	*/
	...
	if(favo!=null){
		for(int i=0; i<favo.length ; i++){
			...
			문자열의 마지막부분에 ,를 제거하기위한 if문을 통해 체크하고있다
			*/
			if(i==favo.length-1){
				favStr += favo[i];
			}else{
	...
	*/
	...
	getParameterNames() : 폼값의 이름을 한꺼번에 받을때 사용하는
	 ...
	 nextElement():받은 폼값중 다음 폼의 이름을반환
	*/
	Enumeration names= request.getParameterNames();
	while(names.hasMoreElements()){
		
		String paraName=(String)names.nextElement();
		System.out.println("파라미터명: "+paraName);
		
		if(paraName.equals("sex") || paraName.equals("favorite")){
			System.out.println("getParameterValue()로 읽기");
